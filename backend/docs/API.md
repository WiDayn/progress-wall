...
1. **密码强度要求**:
   ...
   - 必须包含至少一个字母（大小写均可）
   - 必须包含至少一个数字
   - 密码在存储前会使用bcrypt进行加密（成本因子为12）

2. **密码重置**:
   - 密码重置功能需要额外的安全验证（如邮箱验证码）
   - 重置后的密码必须符合上述强度要求
   - 建议定期更换密码以提高账户安全性
...
1. **敏感信息处理**:
   - 所有接口返回的用户对象中，密码字段已被清除，不会在响应中返回
   - 密码在数据库中以哈希值形式存储，无法直接查看明文

2. **操作审计**:
   - 敏感操作（如账户禁用、密码重置）建议记录操作日志
   - 建议对敏感操作进行频率限制，防止恶意操作
...
7. **密码安全**: 注册和密码相关操作必须符合密码强度要求，系统会自动验证密码格式
